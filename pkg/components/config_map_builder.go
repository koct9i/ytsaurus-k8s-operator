...
import (
	...
	"github.com/ytsaurus/ytsaurus-k8s-operator/pkg/yordered"
)
...
func mergeYSONMaps(dst, src *yordered.Map) *yordered.Map {
	for _, key := range src.Keys() {
		srcValue, _ := src.Get(key)
		dstValue, exists := dst.Get(key)
		if exists {
			dstMap, dstIsMap := dstValue.(*yordered.Map)
			srcMap, srcIsMap := srcValue.(*yordered.Map)
			if dstIsMap && srcIsMap {
				srcValue = mergeYSONMaps(dstMap, srcMap)
			}
		}
